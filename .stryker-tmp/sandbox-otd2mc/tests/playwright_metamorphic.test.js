...
test('MR5: Order independence - Task retrieval should be consistent', async ({ request }) => {
    ...
    // 🔹 Instead of comparing exact counts, compare task IDs to ensure they are the same set
    const taskIds1 = tasks1.map(task => task._id).sort();
    const taskIds2 = tasks2.map(task => task._id).sort();
